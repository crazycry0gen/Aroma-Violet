namespace Aroma_Violet.Controllers
{
    ...
    public class ReportingController : Controller
    {
        ...
        public ActionResult DebitOrderExcelExport()
        {
            ...
            var sql = "select  '' A,'' B,Client.ClientSurname,	client.ClientInitials,	isnull(contact.ContactName,''),'',	Bank.BankName,	Branch.BranchName,	Branch.BranchCode,	isnull(Client.IDNumber + ':ID',''), BankingDetail.AccountNumber,	AccountType.AccountTypeName,	'' M,	'' N,	'' O,	'' P,	sum(finJournal.Amount) from finClientAccount left join finJournal on finJournal.AccountID = finClientAccount.ClientAccountId left join Client on client.ClientId = finClientAccount.ClientID left join Contact on Client.ClientId = Contact.ClientID and ContactTypeID = 1 left join BankingDetail on BankingDetail.ClientID = client.ClientId left join Bank on BankingDetail.BankID = Bank.BankId left join Branch on BankingDetail.BranchID = Branch.BranchId left join AccountType on AccountType.AccountTypeId = BankingDetail.AccountTypeID where  (Contact.Active = 1 or contact.Active is null) and finClientAccount.AccountId = '751FE521-3561-E511-80C3-2047477CE07A' group by Client.ClientSurname, 	client.ClientInitials, 	contact.ContactName, 	Bank.BankName, 	Branch.BranchName, Branch.BranchCode, Client.IDNumber, BankingDetail.AccountNumber, AccountType.AccountTypeName having sum(finJournal.Amount) <> 0";
